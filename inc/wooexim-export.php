...
class Woo_wooexim_export {
	...
	public function add_menu_link() {
		add_submenu_page('wooexim-dashboard','WOOEXIM Export Product', 'Export','manage_options','wooexim-export',	array( $this, 'wooexim_page_export' ));
		add_submenu_page('wooexim-dashboard','WOOEXIM Product Archive', 'Archive','manage_woocommerce','wooexim-archive',	array( $this, 'wooexim_page_archive' ));
		add_submenu_page('wooexim-dashboard','WOOEXIM Settings', 'Settings','manage_woocommerce','wooexim-settings',	array( $this, 'wooexim_page_settings' ));
	}
	
	public function wooexim_page_export( $tab = false ){
		...
		$html = '';
		$html .=  '
		<div class="wooexim-wrap">
			<div id="icon-woocommerce" class="icon32 icon32-woocommerce-importer"><br></div>
			<div class="wooexim-wrap-in">
				<img src="' . WOOEXIM_IMG_URL . 'thumb.jpg">			
				<h2>WOOEXIM &raquo; Export Product</h2>
				<p>Developed by <a href="http://wooexim.com" target="_blank">WOOEXIM</a> Version: 2.0.2</p>
				<p>Quick and easy plugin for WooCommerce export-import.</p>
			</div>
			<div id="content">
	...
	public function wooexim_page_archive( $tab = false ){
		ini_set('max_execution_time', 3000);
		$this->wxe_set_archive_table();
		$show_settings = ''; $show_archive='';$show_export='';
		if( $this->wxe_generate_data_for_export())
		...
		<div class="wooexim-wrap">
			...
			<div class="wooexim-wrap-in">
				<img src="' . WOOEXIM_IMG_URL . 'thumb.jpg">			
				<h2>WOOEXIM &raquo; Export Archive</h2>
				<p>Developed by <a href="http://wooexim.com" target="_blank">WOOEXIM</a> Version: 2.0.2</p>
				<p>Quick and easy plugin for WooCommerce export-import.</p>
			</div>
			<div id="content">
	...
	public function wooexim_page_settings( $tab = false ){
		ini_set('max_execution_time', 3000);
	...
	function get_export_tab_data(){
		...
		<div class="overview">
			<div class="overview-left-1">
				...
				<a class="button-primary all_export_btn export_btn" href="' . wp_nonce_url(get_admin_url().'admin.php?page=wooexim-export&tab=export&export=all_products') . '">Export All Products</a>
					<div class="all_product_exporting_loader">
						<img src="'.WOOEXIM_IMG_URL.'animated_loading.gif" /><span>Exporting All Product Data Please Wait ... </span>
					</div>
			</div>
	
			<div class="overview-right-1">
				<h2>Export Products by Category</h2>
				...
				<a class="button-primary  cat_export_btn export_btn" href="' . wp_nonce_url(get_admin_url().'admin.php?page=wooexim-export&tab=export&export=selected_category') . '">Export Category Products</a>
					<a href="#export-products" class="button export_category_settings">Choose Categories</a>
					<div class="cat_product_exporting_loader">
						<img src="'.WOOEXIM_IMG_URL.'animated_loading.gif" /><span>Exporting Please Wait ... </span>
					</div>
	...
	function wxe_get_all_woo_products(){
		...
		if( $query->have_posts() ){
			...
			<input type="submit" name="submit" value="Save" class="button button-primary save_button" onclick="return save_product()">
	...
	function wxe_get_all_woo_categories(){
		...
		{	
			...
			$data .=  '<div class="clr"></div></ul>
			<input type="submit" name="submit" value="Save" class="button button-primary save_button" onclick="return save_category()">
	...
	function wxe_get_setting_tab_data(){
		if( wp_verify_nonce( $_REQUEST['_wpnonce'] ) && isset( $_REQUEST['save_settings'] ) )
		{
			$prefix_name = sanitize_text_field($_REQUEST['prefix_name']);
			$author_name = sanitize_text_field($_REQUEST['author_name']);
			$subject_name = sanitize_text_field($_REQUEST['subject_name']);
			$description_archive = sanitize_text_field($_REQUEST['description_archive']);
			$field_separator = sanitize_text_field($_REQUEST['field_separator']);
			$hierarchy_separator = sanitize_text_field($_REQUEST['hierarchy_separator']);
		...
		$html = '
			<div class="settings_tab">
				<form method="post" action="' . wp_nonce_url(get_admin_url().'admin.php?page=wooexim-settings') . '">
				<table>
					<tr>
	...
	function wxe_generate_data_for_export(){
		if( wp_verify_nonce( $_REQUEST['_wpnonce'] ) && isset( $_REQUEST['export']) )
		{
